...
export function ipcRenderer(vueInstance) {
  ...
  if (window.electron) {
    try {
      const { ipcRenderer, os } = window.electron;

      document.body.setAttribute('data-electron-os', os.platform());
      ...
      console.error('Error loading Electron or OS modules in renderer:', e);
      // Handle the error, maybe log it or show a message
    }
  } else {
    console.log('Not in Electron environment, skipping ipcRenderer setup.');
  }
}
